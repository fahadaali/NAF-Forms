...
export async function GET(
  ...
  /* ═══ الخلية نصّ لا صيغة ═══
     ...
     الاقتباس وحده لا يمنع Excel من تفسير خلية تبدأ بـ`=` أو `+` أو `-`
     أو `@` صيغةً. فمن كتب `=HYPERLINK("http://…")` في حقل نصّي يجعل
     المشرف ينفّذها بفتحه الملف — وقيم هذا التصدير كلها من معبّئي النموذج.

     والعلاج المعتاد: تُسبَق بعلامة اقتباس مفردة، فتقرؤها الجداول نصًّا
     ولا تعرضها. وتُضاف كذلك لمن يبدأ بمِحرف تحكّم (tab أو CR) لأنه يُزاح
     في بعض المحلّلات فيكشف ما بعده.

     ولا يُطبَّق على `xlsx`: `aoa_to_sheet` تكتب الخلية بنوع نصّي صراحةً. */
  const esc = (v: any) => {
    let s = String(v ?? "");
    if (/^[=+\-@\t\r]/.test(s)) s = `'${s}`;
    return `"${s.replace(/"/g, '""')}"`;
