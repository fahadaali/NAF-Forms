...
الربط `BUCKET` مضبوط في `wrangler.toml` ويكفي وحده.

**وسير النشر يتولّى هذا كلّه:** ينشئ الحاوية إن لم تكن موجودة، ويفعّل
رابطها العام (`r2.dev`)، ويضبطه سرًّا باسم `R2_PUBLIC_URL`. فلا خطوة يدوية
هنا في المسار المعتاد.

### أين تُقدَّم المرفقات، ولماذا لا يضرّ الفرق

| الحال | المسار | الترويسات |
|---|---|---|
| `R2_PUBLIC_URL` مضبوط | `https://pub-….r2.dev/<key>` | ما تحمله R2 |
| غير مضبوط | `‎/uploads/<key>` عبر الـ Worker | `attachment` + `nosniff` + `CSP: sandbox` |

المسار الثاني أشدّ، لكن الأول **ليس ثغرة**: `r2.dev` أصلٌ آخر، فمستندٌ
خبيث فيه يعمل في سياقه لا في سياق `forms.naflaw.sa` — لا يقرأ كوكيز
المنصة ولا يمسّ جلسة المشرف. والطبقة الأولى (منع رفع SVG وHTML نوعًا
وامتدادًا في `‎/api/upload`) تعمل في الحالين.

> وإن أردت دومينًا مخصّصًا بدل `r2.dev`، ضع رابطه في `R2_PUBLIC_URL`.
> وإن أردت أشدّ الحالين، احذف السرّ ولا تفعّل الرابط العام — تُقدَّم
> المرفقات عبر الـ Worker.
