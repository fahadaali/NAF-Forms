...
## ⚠️ متطلّب مهم: إصدار Next.js
إصدار `@opennextjs/cloudflare` الحالي (1.20+) **يتطلّب Next.js 15+**، بينما
المشروع على Next 14. قبل النشر على Workers يلزم **ترقية Next إلى 15** (تغييرات
كاسرة: `cookies()`/`headers()`/`params` أصبحت غير متزامنة). البدائل:
- **ترقية إلى Next 15** ثم متابعة الخطوات أدناه (الأنظف طويل المدى).
- أو استخدام مُحوّل أقدم يدعم Next 14 (غير مُوصى به — متوقّف الصيانة).
- أو نشر التطبيق كتطبيق **Node عادي** (Render/Fly/Railway) مع **R2 عبر واجهة
  S3** (مُفعّل الآن في `src/lib/storage.ts`) — يعمل فورًا لكن دون D1.

> رفع الملفات إلى R2، والمصادقة (Web Crypto)، وطبقة الهجرات جاهزة الآن.
> الجزء الذي يتطلّب Next 15 هو تشغيل التطبيق نفسه فوق Workers عبر OpenNext.

## 1) مُحوّل التشغيل على Cloudflare (بعد ترقية Next 15)
```bash
npm install --save-dev @opennextjs/cloudflare
```
أضف سكربتات إلى `package.json`:
```jsonc
"scripts": {
...
## 6) ربط Prisma بـ D1 (تعديل مطلوب مرة واحدة)
التطبيق محليًا يستخدم `src/lib/prisma.ts` (Node). على Workers استخدم الرابط:
```ts
...
```
> جاهز في `src/lib/prisma-d1.ts`. يمكن جعل `src/lib/prisma.ts` يختار تلقائيًا
> بين Node و D1 حسب البيئة عند التشغيل الأول على حسابك (يحتاج تحققًا فعليًا).
